...
const schema = {
    ...
    mqttServiceTopic: {
        ...
        format: String,
        default:
            '/test/vhs/spacebus/status/services/easyappointments-mask-schedule-monitor',
...
type ConfigSchemaKey = keyof typeof schema
...
if (config.get('checkInterval') < 1000)
    config.set('checkInterval', config.get('checkInterval') * 1000)

const dumpFields: ConfigSchemaKey[] = [
    'env',
    'baseUrl',
    'apiKey',
    'categoryId',
...
dumpFields.forEach((key: unknown) => {
    debug(key, '\t', config.get(key as ConfigSchemaKey))
})
